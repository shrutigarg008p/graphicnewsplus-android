...
class NewsDetails extends StatefulWidget {
  ...
  const NewsDetails({Key? key, this.newsId, this.title}) : super(key: key);
...
class _NewsDetailsState extends State<NewsDetails> {
  ...
  void initState() {
    ...
    getapi(backReloadApi: true);
    fireScreenViewevent();
  }

  void update() {
    ShowAds.setFullAds(context, BaseKey.NEWS_DETAILS);
  }
  ...
  getLocalDatabase() {
    tempId = StringUtil.tempValue(widget.newsId, BaseKey.Publish_NewsPaper);
  ...
  getapi({bool? backReloadApi}) {
    ...
    HttpObj.instance
        .getClient()
        .getNewsPaperDetailPage(
            widget.newsId!, randomNumber.toString() /*BaseKey.base64_NO*/)
        .then((it) {
      setState(() {
        setException(false);
        ...
        if (it.dATA != null) {
          ...
          //debugPrint(it.dATA!.post!.subscribed.toString() +
          //  " Subscribed Value Response from API ");
          //debugPrint(it.toJson().toString(), wrapWidth: 500);

          printWrapped(it.dATA!.post!.subscribed.toString() +
              " Subscribed Value Response from API ");
                      ...
                      child: ListView(
                        children: [
                          ...
                          ShowAds.setNativeAds(
                              context, BaseKey.NEWS_DETAILS_MEDIUM),
                          if (magazineDetailResponse != null)
  ...
  checkUid(BuildContext context) {
    print("UID");
    print(magazineDetailResponse!.dATA!.post!.uId);
    return Column(
      children: <Widget>[
        if (datapdf != null && (datapdf?.tempId! == tempId!)) ...[
          ...
          CommonFilledBtn(
            ...
            onTap: () async {
              fireMagzinePreviewEvent(
                  widget.newsId.toString(), widget.title, "newaspaper");
              CommonSetEvent.setEvent("reading", "newspaper",
                  id: widget.newsId);
          ...
          CommonDownload(
            ...
            isold: magazineDetailResponse!.dATA!.isold!,
            PublishTag: BaseKey.Publish_NewsPaper,
...
//this is for news details
                        ...
                        Navigator.push(
                            ...
                            MaterialPageRoute(
                                builder: (context) => NewsDetails(
                                      newsId: magazineDetailResponse
                                          .dATA!.related![index].id,
                                      title: magazineDetailResponse
                                          .dATA!.related![index].title!,
                                    ))),
                        fireItemViewevent(
                            magazineDetailResponse.dATA!.related![index].id
                                .toString(),
                            magazineDetailResponse.dATA!.related![index].title!)
                      },
                      child: Padding(
  ...
  fireScreenViewevent() async {
    ...
    await FirebaseAnalytics.instance.logEvent(
      name: "screen_view",
      parameters: {
        "screen_name": "NewPapersDetailsScreen",
      },
    );
  }
