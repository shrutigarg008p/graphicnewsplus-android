...
class PromotedContentListing extends StatefulWidget {
  @override
  _PromotedContentListingState createState() => _PromotedContentListingState();
}

class _PromotedContentListingState extends State<PromotedContentListing> {
  ...
  void update() {
    ShowAds.setFullAds(context, BaseKey.PROMOTED_CONTENT_LISTING);
  }
  ...
  getapi() {
    HttpObj.instance.getClient().getPromotedListingPage(_Firstpage).then((it) {
      setState(() {
        setException(false);
  ...
  void _loadMore() async {
    _page = _page + 1;
    loadMoreData(true);
    HttpObj.instance.getClient().getPromotedListingPage(_page).then((it) {
      if (mounted) {
        setState(() {
  ...
  _buildNewsList(BuildContext context) {
    return CommonWidget(context).getObjWidget(
        ...
        exception,
        _buildPromotedList(context, promotedListingResponse),
        retryCallback());
  }

  _buildPromotedList(
      BuildContext context, PromotedListingResponse? promotedListingResponse) {
    if (promotedListingResponse == null) {
      return Container();
    }
    return SafeArea(
  ...
  getBody() {
    return Scaffold(
        key: scaffoldKey,
        appBar: HeaderWidget.appHeader(BaseConstant.PROMOTED_CONTENT, context),
